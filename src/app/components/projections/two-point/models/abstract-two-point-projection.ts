...
import { Coordinate, Distance, GreatCircle, Bearing } from '@dvanauken/sphere';
...
export abstract class AbstractTwoPointProjection {
    ...
    protected validatePoint(point: Point): Point {
        // Sphere's Coordinate constructor already handles validation
        const coord = new Coordinate(point.latitude, point.longitude);
        return {
            latitude: Number(coord.latitude.toFixed(3)),
            longitude: Number(coord.longitude.toFixed(3))
        };
    }
    ...
    protected calculateDistance(p1: Point, p2: Point): number {
        const coord1 = new Coordinate(p1.latitude, p1.longitude);
        const coord2 = new Coordinate(p2.latitude, p2.longitude);
        const distance = GreatCircle.from(coord1).to(coord2).distance();
        return distance.inKilometers(); // Since your original used km
    }

    // Replace calculateBearing with Sphere's Bearing
    protected calculateBearing(p1: Point, p2: Point): number {
        const coord1 = new Coordinate(p1.latitude, p1.longitude);
        const coord2 = new Coordinate(p2.latitude, p2.longitude);
        return Bearing.from(coord1).to(coord2).initial().degrees;
    }
